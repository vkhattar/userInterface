...
describe('First unit test', function() {
    ...
    it('Listener unit test for ActivityModel', function() {
        var ActvityModel = new ActivityStoreModel();
        
        var firstData = sinon.spy();
        var secondData = sinon.spy();
        var thirdData = sinon.spy();

        ActivityModel.addActivityDataPoint(firstData);
        ActivityModel.addActivityDataPoint(secondData);
        ActivityModel.addActivityDataPoint(thirdData);



        // expect(firstListener.called, 'ActivityModel listener should be called').to.be.ok;
        // expect(firstListener.calledWith('firstData'), 'GraphModel argument verification').to.be.true;
